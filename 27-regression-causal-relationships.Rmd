...
```{r min-dag, echo=FALSE, message=FALSE, out.width="71%", fig.cap="To estimate the effect of X on Y, we must control for P and Z"}
library("tidyverse")
g_omitted <- ggplot() +
...
g_rdd1 <- mlda %>%
  ...
  labs(x="Age", y="Deaths per 100,000 persons",
       title=expression(Y[i]==a[1]+a[2]*X[i]+a[3]*T[i]+u)) +
...
g_rdd2 <- mlda %>%
  ...
  geom_function(fun=function(x) coefs2[[1]] + coefs2[[3]] + coefs2[[2]]*x + coefs2[[4]]*x,
                xlim=c(21,23), linetype="dashed", color="#00BFC4") +
  labs(x="Age", y="Deaths per 100,000 persons",
       title=expression(Y[i]==b[1]+b[2]*X[i]+b[3]*T[i]+b[4]*(X[i]*T[i])+v)) +
  theme(text=element_text(size=11))
(g_rdd1 + g_rdd2)
```
...
```{r 27_regression_causal_relationships_ex_6_pdf, echo=FALSE, results='asis'}
if (knitr::is_latex_output()) {
  ex_items <- list(
    list(
      q = "A researcher wants to study the effect of a job training program on employment.\n\n(a) Why is it difficult to conduct a randomized experiment?\n\n(b) Describe a quasi-experimental approach.\n\n(c) What assumption is needed for the quasi-experiment to be valid?",
      a = "(a) ethical and practical constraints prevent random assignment, (b) exploit an unintended or administrative mechanism that randomizes treatment, (c) the mechanism must be independent of potential outcomes (as-good-as-random assignment).\n\n"
    ),
    list(
      q = "A regression of wages on education gives \\(\\hat{b}=100\\) . Adding family background as a control reduces \\(\\hat{b}\\) to 60.\n\n(a) What does the decrease in (hat{b}) suggest?\n\n(b) What is the name of this problem?\n\n(c) Is the original estimate biased upward or downward?",
      a = "(a) family background is a confounder that was omitted from the original model, (b) omitted variable bias, (c) upward (the original estimate confounded education with family background).\n\n"
    ),
    list(
      q = "A scholarship is awarded to students scoring \\(\\geq70\\) on a test. Students just above 70 earn more later than students just below 70.\n\n(a) What is the threshold in this regression discontinuity design?\n\n(b) Why is this a quasi-experiment?\n\n(c) What is the key identifying assumption?",
      a = "(a) score of 70, (b) assignment near the threshold is as good as random, (c) no other factor changes discontinuously at the threshold.\n\n"
    ),
    list(
      q = "Geographic proximity to a university is used as an instrumental variable \\(Z\\) for education \\(X\\) in a wage regression.\n\n(a) What condition must the instrument (Z) satisfy (relevance)?\n\n(b) What is the exclusion restriction?\n\n(c) Why would proximity to a university affect wages only through education?",
      a = "(a) \\(Z\\) must be correlated with \\(X\\) (proximity predicts education), (b) \\(Z\\) affects wages only through education (not directly), (c) proximity to a university does not directly affect wages apart from the education it encourages.\n\n"
    ),
    list(
      q = "Region A introduces a minimum wage policy; region B does not. Employment before: A \\(=100\\) , B \\(=120\\) . Employment after: A \\(=105\\) , B \\(=125\\) .\n\n(a) What is the change in employment in region A?\n\n(b) What is the change in employment in region B?\n\n(c) What is the difference-in-differences estimate of the policy effect?",
      a = "(a) \\(\\Delta A=5\\) , (b) \\(\\Delta B=5\\) , (c) \\(\\hat{\\tau}=5-5=0\\) (no estimated effect).\n\n"
    ),
    list(
      q = "The DiD estimator is \\(\\hat{\\tau}=(\\bar{Y}_{B,\\text{after}}-\\bar{Y}_{B,\\text{before}})-(\\bar{Y}_{K,\\text{after}}-\\bar{Y}_{K,\\text{before}})\\) . Given: \\(\\bar{Y}_{B,\\text{after}}=80\\) , \\(\\bar{Y}_{B,\\text{before}}=60\\) , \\(\\bar{Y}_{K,\\text{after}}=70\\) , \\(\\bar{Y}_{K,\\text{before}}=55\\) :\n\n(a) Calculate the change in the treatment group.\n\n(b) Calculate the change in the control group.\n\n(c) Calculate (hat{tau}) .",
      a = "(a) \\(80-60=20\\) , (b) \\(70-55=15\\) , (c) \\(\\hat{\\tau}=20-15=5\\) .\n\n"
    ),
    list(
      q = "A DiD study uses cities A (treated) and B (control).\n\n(a) What is the parallel trends assumption?\n\n(b) Why is this assumption untestable for the post-treatment period?\n\n(c) How can we check it using pre-treatment data?",
      a = "(a) without treatment, both groups would have followed the same trend over time, (b) we cannot observe the counterfactual post-treatment trend for the treated group, (c) verify that the two groups had similar trends before the treatment.\n\n"
    ),
    list(
      q = "A tax benefit is given to firms with fewer than 50 employees. A researcher compares firms with 48–49 employees to firms with 50–51 employees.\n\n(a) What is the treatment in this regression discontinuity design?\n\n(b) Why compare firms just around the threshold?\n\n(c) What assumption makes the two groups comparable?",
      a = "(a) the tax benefit, (b) firms just above and below the threshold are otherwise similar, (c) firms are comparable on either side of the threshold (continuity assumption).\n\n"
    ),
    list(
      q = "Treated group outcomes: before \\(=50\\) , after \\(=70\\) . Control group outcomes: before \\(=40\\) , after \\(=50\\) .\n\n(a) What is the change in the treated group?\n\n(b) What is the change in the control group?\n\n(c) What is the DiD estimate?",
      a = "(a) \\(70-50=20\\) , (b) \\(50-40=10\\) , (c) \\(\\hat{\\tau}=20-10=10\\) .\n\n"
    ),
    list(
      q = "A regression shows that education has an estimated effect of \\(\\hat{b}=100\\) on wages.\n\n(a) Why can we not necessarily interpret this as a causal effect?\n\n(b) Name one source of endogeneity.\n\n(c) What quasi-experimental method could address this?",
      a = "(a) education may be correlated with unobserved ability or family background, (b) omitted variable bias / self-selection, (c) instrumental variable, regression discontinuity, or difference-in-differences.\n\n"
    ),
    list(
      q = "Explain the concepts:\n\n(a) What is omitted variable bias?\n\n(b) What is endogeneity?\n\n(c) What does the error term in a regression represent?",
      a = "(a) bias from omitting a variable correlated with both \\(X\\) and \\(Y\\) , (b) \\(X\\) is correlated with the error term, (c) all unobserved factors that affect \\(Y\\) and are not included in the model.\n\n"
    ),
    list(
      q = "A new school policy is introduced in some municipalities but not others. The introduction was determined by a random administrative lottery.\n\n(a) What type of quasi-experimental design is this?\n\n(b) What makes the lottery useful for causal identification?\n\n(c) What assumption must hold for the non-lottery municipalities to serve as a valid control group?",
      a = "(a) natural experiment / randomized lottery, (b) the lottery randomly assigns treatment, removing selection bias, (c) that municipalities without the lottery would have had parallel trends to lottery municipalities absent the policy.\n\n"
    )
  )
...
var ex=[
  ...
  {q:'<p>Treated group outcomes: before \\(=50\\) , after \\(=70\\) . Control group outcomes: before \\(=40\\) , after \\(=50\\) .</p><ol type="a"><li>What is the change in the treated group?</li><li>What is the change in the control group?</li><li>What is the DiD estimate?</li></ol>',a:'<p>(a) \\(70-50=20\\) , (b) \\(50-40=10\\) , (c) \\(\\hat{\\tau}=20-10=10\\) .</p>'},
