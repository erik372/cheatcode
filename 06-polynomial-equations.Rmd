...
```{r andragradsekvationer, echo=FALSE, out.width="71%", fig.height=9.8, fig.cap="Examples of quadratic equations"}
library("tidyverse")
library("patchwork")
the_g6 <- ggplot(tibble(x=-5:5), aes(x)) +
  geom_vline(xintercept=0, alpha=.3) +
  geom_hline(yintercept=0, alpha=.3) +
  scale_x_continuous(breaks=-5:5)
th6 <- theme(axis.title.y=element_text(angle=0), text=element_text(size=11))

g_x2_a <- the_g6 + geom_function(fun=function(x) x^2, color="#F8766D") + ggtitle(bquote("(a) y ="~x^2)) + th6
g_x2_b <- the_g6 + geom_function(fun=function(x) x^2-9, color="#F8766D") + ggtitle(bquote("(b) y ="~x^2-9)) + th6
g_x2_c <- the_g6 + geom_function(fun=function(x) 4-x^2, color="#F8766D") + ggtitle(bquote("(c) y ="~4-x^2)) + th6
g_x2_d <- the_g6 + geom_function(fun=function(x) x^2+9, color="#F8766D") + ggtitle(bquote("(d) y ="~x^2+9)) + th6
g_x2_e <- the_g6 + geom_function(fun=function(x) (2-x)^2, color="#F8766D") + ggtitle(bquote("(e) y ="~(2-x)^2)) + th6
g_x2_f <- the_g6 + geom_function(fun=function(x) (x+3)^2, color="#F8766D") + ggtitle(bquote("(f) y ="~(x+3)^2)) + th6

g_x2_plots <- (g_x2_a + g_x2_b) / (g_x2_c + g_x2_d) / (g_x2_e + g_x2_f)
print(g_x2_plots)
```
...
$$
\begin{align}
x^{*} & =-\frac{p}{2}\pm\sqrt{\left(\frac{p}{2}\right)^{2}-q} (\#eq:pq-formeln)\\
...
```{r ex-3e-och-4e-grads-poly, echo=FALSE, out.width="71%", fig.cap="Examples of third- and fourth-degree polynomials"}
p6b <- ggplot(tibble(x=-10:10), aes(x)) +
  geom_vline(xintercept=0, alpha=.3) +
  geom_hline(yintercept=0, alpha=.3) +
  theme(axis.title.y=element_text(angle=0), text=element_text(size=11)) +
  scale_y_continuous(labels=function(x) format(x, big.mark=" ", scientific=FALSE))

g_poly_1 <- p6b + geom_function(fun=function(x) 2*x^3-x^2+7, color="#F8766D") +
  labs(title=bquote("(a) y ="~2*x^3-x^2+7))
g_poly_2 <- p6b + geom_function(fun=function(x) 10*x^2+5*x^3-x^4, color="#F8766D") +
  xlim(0,8) +
...
```{r fler-exempel-pa-polynom, echo=FALSE, out.width="71%", fig.cap="More examples of polynomials"}
p6a <- ggplot(tibble(x=0:10), aes(x)) +
  geom_vline(xintercept=0, alpha=.3) +
  geom_hline(yintercept=0, alpha=.3) +
  scale_y_continuous(labels=function(x) format(x, big.mark=" ", scientific=FALSE))

gx2a6 <- p6a + geom_function(fun=function(x) 75-x^2, color="#F8766D") + xlim(-10,10) + labs(title="(a)")
gx2b6 <- p6a + geom_function(fun=function(x) -10+10*x^2-x^3, color="#F8766D") + xlim(-4,12) + ylim(-75,200) + labs(title="(b)")
gx2c6 <- p6a + geom_function(fun=function(x) 100*x-100*x^2+x^3, color="#F8766D") + xlim(-50,125) + labs(title="(c)")
gx2d6 <- p6a + geom_function(fun=function(x) 10*x^2+10*x^3-x^4, color="#F8766D") + xlim(-4,12) + labs(title="(d)")

print((gx2a6 + gx2b6) / (gx2c6 + gx2d6))
```
...
```{r 06_polynomial_equations_ex_1_pdf, echo=FALSE, results='asis'}
if (knitr::is_latex_output()) {
  ex_items <- list(
    list(
      q = "For each expression, state whether it is a polynomial. If yes, state its degree:\n\n(a) (x^{3}-2x+5)\n\n(b) (x^{-1}+3)\n\n(c) (4x^{2}-x+7)",
      a = "(a) Yes, degree 3, (b) No (negative exponent), (c) Yes, degree 2\n\n"
    ),
    list(
      q = "Solve:\n\n(a) (x^{2}=25)\n\n(b) (x^{2}-16=0)\n\n(c) (4x^{2}=36)",
      a = "(a) \\(x=\\pm5\\) , (b) \\(x=\\pm4\\) , (c) \\(x=\\pm3\\)\n\n"
    ),
    list(
      q = "Factorize and solve:\n\n(a) (x^{2}-5x+6=0)\n\n(b) (x^{2}+2x-8=0)\n\n(c) (x^{2}-7x+12=0)",
      a = "(a) \\((x-2)(x-3)=0\\) , \\(x_{1}^{*}=2,\\,x_{2}^{*}=3\\) , (b) \\((x+4)(x-2)=0\\) , \\(x_{1}^{*}=-4,\\,x_{2}^{*}=2\\) , (c) \\((x-3)(x-4)=0\\) , \\(x_{1}^{*}=3,\\,x_{2}^{*}=4\\)\n\n"
    ),
    list(
      q = "Factorize using special identities:\n\n(a) (x^{2}-9)\n\n(b) (x^{2}+6x+9)\n\n(c) (x^{2}-10x+25)",
      a = "(a) \\((x+3)(x-3)\\) , (b) \\((x+3)^{2}\\) , (c) \\((x-5)^{2}\\)\n\n"
    ),
    list(
      q = "Use the rational root theorem to find candidates, then solve:\n\n(a) (x^{2}-x-6=0)\n\n(b) (x^{2}+x-12=0)\n\n(c) (x^{2}-3x-10=0)",
      a = "(a) \\(x_{1}^{*}=3,\\,x_{2}^{*}=-2\\) , (b) \\(x_{1}^{*}=3,\\,x_{2}^{*}=-4\\) , (c) \\(x_{1}^{*}=5,\\,x_{2}^{*}=-2\\)\n\n"
    ),
    list(
      q = "Solve using the quadratic formula \\(x^{*}=-p/2\\pm\\sqrt{(p/2)^{2}-q}\\) where \\(p=b/a\\) and \\(q=c/a\\) :\n\n(a) (x^{2}-5x+4=0)\n\n(b) (x^{2}+3x-10=0)\n\n(c) (2x^{2}-7x+3=0)",
      a = "(a) \\(p=-5,\\,q=4\\) : \\(x^{*}=5/2\\pm3/2\\) , \\(x_{1}^{*}=4,\\,x_{2}^{*}=1\\) , (b) \\(p=3,\\,q=-10\\) : \\(x^{*}=-3/2\\pm7/2\\) , \\(x_{1}^{*}=2,\\,x_{2}^{*}=-5\\) , (c) \\(p=-7/2,\\,q=3/2\\) : \\(x^{*}=7/4\\pm5/4\\) , \\(x_{1}^{*}=3,\\,x_{2}^{*}=1/2\\)\n\n"
    ),
    list(
      q = "For each equation, determine whether it has two, one, or no real solutions by calculating \\((p/2)^{2}-q\\) :\n\n(a) (x^{2}-4x+5=0)\n\n(b) (x^{2}-4x+4=0)\n\n(c) (x^{2}-4x+3=0)",
      a = "(a) \\((p/2)^{2}-q=4-5=-10\\) : two solutions, \\(x_{1}^{*}=3,\\,x_{2}^{*}=1\\)\n\n"
    ),
    list(
      q = "For the cubic polynomial \\(x^{3}-6x^{2}+11x-6=0\\) :\n\n(a) Test (x=1) as a potential root.\n\n(b) Divide (x^{3}-6x^{2}+11x-6) by ((x-1)) and write down the resulting quadratic polynomial.\n\n(c) Solve the quadratic polynomial from (b) to find all three solutions.",
      a = "(a) \\(1-6+11-6=0\\) , confirmed root, (b) \\(x^{3}-6x^{2}+11x-6=(x-1)(x^{2}-5x+6)\\) , (c) \\((x-2)(x-3)=0\\) : \\(x_{1}^{*}=1,\\,x_{2}^{*}=2,\\,x_{3}^{*}=3\\)\n\n"
    ),
    list(
      q = "Factorize and find all solutions:\n\n(a) (x^{3}-x=0)\n\n(b) (x^{3}+3x^{2}-4x=0)\n\n(c) (x^{3}-4x^{2}+4x=0)",
      a = "(a) \\(x(x+1)(x-1)=0\\) : \\(x_{1}^{*}=0,\\,x_{2}^{*}=-1,\\,x_{3}^{*}=1\\) , (b) \\(x(x+4)(x-1)=0\\) : \\(x_{1}^{*}=0,\\,x_{2}^{*}=-4,\\,x_{3}^{*}=1\\) , (c) \\(x(x-2)^{2}=0\\) : \\(x_{1}^{*}=0,\\,x_{2}^{*}=2\\) (double root)\n\n"
    ),
    list(
      q = "Solve using any appropriate method:\n\n(a) (3x^{2}-12=0)\n\n(b) (x^{2}-2x-15=0)\n\n(c) (x^{3}-2x^{2}-x+2=0) (hint: test (x=1) )",
      a = "(a) \\(x^{2}=4,\\,x=\\pm2\\) , (b) \\((x-5)(x+3)=0\\) : \\(x_{1}^{*}=5,\\,x_{2}^{*}=-3\\) , (c) \\((x-1)(x-2)(x+1)=0\\) : \\(x_{1}^{*}=1,\\,x_{2}^{*}=2,\\,x_{3}^{*}=-1\\)\n\n"
    )
  )
