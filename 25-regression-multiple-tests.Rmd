...
```{r 25_regression_multiple_tests_ex_26_pdf, echo=FALSE, results='asis'}
if (knitr::is_latex_output()) {
  ex_items <- list(
    list(
      q = "Multiple regression with \\(n = 100\\), \\(k = 3\\) regressors, \\(R^2 = 0.45\\). F-statistic: \\(F = R^2(n-k-1)/[(1-R^2)k]\\).\n\n(a) Compute \\(F\\).\n\n(b) Critical value \\(F^*(3,96) \\approx 2.70\\). Reject \\(H_0\\)?\n\n(c) What is \\(H_0\\) in the F-test for overall significance?",
      a = "(a) \\(F = 0.45 \\times 96 / (0.55 \\times 3) \\approx 26.2\\).\n\n(b) \\(26.2 > 2.70\\), so yes, reject \\(H_0\\).\n\n(c) \\(H_0: b_1 = b_2 = b_3 = 0\\) — all slope coefficients are simultaneously equal to zero."
    ),
    list(
      q = "Estimated model: \\(\\hat{y} = 2 + 3x_1 + x_2 - 2x_3\\), \\(se(\\hat{b}_1) = 1\\), \\(se(\\hat{b}_2) = 0.5\\), \\(se(\\hat{b}_3) = 0.8\\).\n\n(a) Compute \\(t_1\\), \\(t_2\\), \\(t_3\\).\n\n(b) Which coefficients are statistically significant at \\(\\alpha = 0.05\\) (critical value \\(|t| > 1.96\\))?\n\n(c) Interpret the significance of \\(\\hat{b}_3 = -2\\).",
      a = "(a) \\(t_1 = 3/1 = 3\\), \\(t_2 = 1/0.5 = 2\\), \\(t_3 = -2/0.8 = -2.5\\).\n\n(b) All three: \\(|t_1| = 3 > 1.96\\), \\(|t_2| = 2 > 1.96\\), \\(|t_3| = 2.5 > 1.96\\).\n\n(c) \\(x_3\\) has a statistically significant negative effect on \\(Y\\): a one-unit increase in \\(x_3\\) is associated with a 2-unit decrease in \\(y\\), holding other variables constant."
    ),
    list(
      q = "A multiple regression has \\(n = 50\\) observations and \\(k = 4\\) regressors.\n\n(a) How many degrees of freedom does the t-test have?\n\n(b) If we add 3 more regressors (total \\(k = 7\\)), what are the new degrees of freedom?\n\n(c) Why does losing degrees of freedom raise the critical t-value?",
