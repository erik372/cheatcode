...
```{r 17_probability_continuous_ex_22_pdf, echo=FALSE, results='asis'}
if (knitr::is_latex_output()) {
  ex_items <- list(
    list(
      q = "\\(X\\) is continuously uniformly distributed on \\([2,8]\\).\n\n(a) What is the probability density (f(x))?\n\n(b) Calculate (E(X)=(a+b)/2).\n\n(c) Calculate (operatorname{var}(X)=(b-a)^{2}/12).",
      a = "(a) \\(f(x)=1/6\\) for \\(x\\in[2,8]\\). (b) \\(E(X)=5\\). (c) \\(\\operatorname{var}(X)=3\\).\n\n"
    ),
    list(
      q = "\\(X\\) is uniformly distributed on \\([0,10]\\).\n\n(a) What is (f(x))?\n\n(b) Calculate (P(3leq Xleq7)).\n\n(c) Calculate (E(X)).",
      a = "(a) \\(f(x)=1/10\\) for \\(x\\in[0,10]\\). (b) \\(P=0.4\\). (c) \\(E(X)=5\\).\n\n"
    ),
    list(
      q = "A continuous random variable has density \\(f(x)=2x\\) for \\(x\\in[0,1]\\).\n\n(a) Verify that (int_{0}^{1}f(x),dx=1).\n\n(b) Calculate (E(X)=int_{0}^{1}xf(x),dx).\n\n(c) Calculate (operatorname{var}(X)=E(X^{2})-[E(X)]^{2}).",
      a = "(a) \\(\\int_{0}^{1}2x\\,dx=[x^{2}]_{0}^{1}=1\\). (b) \\(E(X)=2/3\\). (c) \\(\\operatorname{var}(X)=1/18\\).\n\n"
    ),
    list(
      q = "A continuous random variable has density \\(f(x)=3x^{2}\\) for \\(x\\in[0,1]\\).\n\n(a) Verify that (int_{0}^{1}f(x),dx=1).\n\n(b) Calculate (E(X)).\n\n(c) What is (P(Xleq0.5))?",
      a = "(a) \\(\\int_{0}^{1}3x^{2}\\,dx=1\\). (b) \\(E(X)=3/4\\). (c) \\(P(X\\leq0.5)=(0.5)^{3}=0.125\\).\n\n"
    ),
    list(
      q = "\\(X\\sim\\text{Exp}(\\lambda=2)\\).\n\n(a) Calculate (E(X)=1/lambda).\n\n(b) Calculate (operatorname{var}(X)=1/lambda^{2}).\n\n(c) Calculate (P(X>1)=e^{-lambdacdot1}).",
      a = "(a) \\(E(X)=0.5\\). (b) \\(\\operatorname{var}(X)=0.25\\). (c) \\(P(X>1)=e^{-2}\\approx0.135\\).\n\n"
    ),
    list(
      q = "\\(X\\sim\\text{Exp}(\\lambda=1)\\).\n\n(a) Calculate (E(X)).\n\n(b) Calculate (P(Xleq1)=1-e^{-1}).\n\n(c) Calculate (P(X>2)=e^{-2}).",
      a = "(a) \\(E(X)=1\\). (b) \\(P(X\\leq1)\\approx0.632\\). (c) \\(P(X>2)=e^{-2}\\approx0.135\\).\n\n"
    ),
    list(
      q = "\\(X\\sim\\chi^{2}(r=4)\\).\n\n(a) Calculate (E(X)=r).\n\n(b) Calculate (operatorname{var}(X)=2r).\n\n(c) What is the chi-squared distribution commonly used for in statistics?",
      a = "(a) \\(E(X)=4\\). (b) \\(\\operatorname{var}(X)=8\\). (c) Testing goodness of fit and independence in contingency tables.\n\n"
    ),
    list(
      q = "\\(X\\sim\\chi^{2}(r=6)\\).\n\n(a) Calculate (E(X)).\n\n(b) Calculate (operatorname{var}(X)).\n\n(c) How is the chi-squared distribution related to the standard normal distribution?",
      a = "(a) \\(E(X)=6\\). (b) \\(\\operatorname{var}(X)=12\\). (c) If \\(Z\\sim N(0,1)\\) then \\(Z^{2}\\sim\\chi^{2}(1)\\); the sum of \\(r\\) squared standard normals follows \\(\\chi^{2}(r)\\).\n\n"
    ),
    list(
      q = "A Gamma distribution has parameters \\(k=4\\) and \\(\\theta=2\\).\n\n(a) Calculate (E(X)=ktheta).\n\n(b) Calculate (operatorname{var}(X)=ktheta^{2}).\n\n(c) Calculate the standard deviation (s(X)).",
      a = "(a) \\(E(X)=8\\). (b) \\(\\operatorname{var}(X)=16\\). (c) \\(s(X)=4\\).\n\n"
    ),
    list(
      q = "\\(X\\sim F(r_{1}=2, r_{2}=3)\\).\n\n(a) How is the F-distribution constructed from chi-squared variables?\n\n(b) What is the F-distribution commonly used for in regression analysis?\n\n(c) What happens to the F-statistic when the regression model has no explanatory power?",
      a = "(a) It is the ratio of two scaled chi-squared variables: \\(S=(X/r_{1})/(Y/r_{2})\\). (b) Testing whether the regression model explains significant variation (F-test). (c) The F-statistic approaches 1 (the ratio of equal chi-squared variables).\n\n"
    ),
    list(
      q = "Explain the difference between discrete and continuous random variables.\n\n(a) Give an example of a discrete random variable.\n\n(b) Give an example of a continuous random variable.\n\n(c) Why is (P(X=x)=0) for any specific value (x) when (X) is continuous?",
      a = "(a) Number of heads in coin flips. (b) Height or waiting time. (c) There are infinitely many values in any interval, so each individual value has zero probability.\n\n"
    ),
    list(
      q = "Two models for waiting times both have \\(E(X)=5\\): Uniform \\([0,10]\\) and \\(\\text{Exp}(\\lambda=0.2)\\).\n\n(a) Calculate (operatorname{var}(X)) for the uniform model.\n\n(b) Calculate (operatorname{var}(X)) for the exponential model.\n\n(c) What is an advantage of the exponential model for modeling waiting times?",
      a = "(a) \\(\\operatorname{var}(X)=(10-0)^{2}/12\\approx8.33\\). (b) \\(\\operatorname{var}(X)=1/\\lambda^{2}=25\\). (c) The exponential distribution is memoryless and allows for long waiting times (heavy right tail).\n\n"
    )
  )
