...
```{r 3d-regression-y-x-z, echo=FALSE, fig.cap="Results from the regression model $y_{i}=a+bx_{i}+cz_{i}+v_{i}$", fig.topcaption=TRUE}
if (knitr::is_html_output()) {
  invisible({
    for(out_dir in c("docs", "_book")) {
      if(dir.exists(out_dir)) {
        dst <- file.path(out_dir, "Eriks_3d_graf")
        if(!dir.exists(dst)) dir.create(dst, recursive=TRUE)
        file.copy("Eriks_3d_graf/3dscatter.html", file.path(dst, "3dscatter.html"), overwrite=TRUE)
        if(dir.exists("Eriks_3d_graf/libs"))
          file.copy("Eriks_3d_graf/libs", dst, recursive=TRUE, overwrite=TRUE)
      }
    }
  })
  knitr::include_url("Eriks_3d_graf/3dscatter.html", height="500px")
} else {
  knitr::include_graphics("figures/g_3d_new2.png")
}
```
...
```{r medelinkomst-kon-6-kommuner, echo=FALSE, out.width="71%", fig.cap="Average earnings for men and women in three countries"}
library("tidyverse")
earnings_df19 <- read_csv("data/ilostat earnings sex.csv", show_col_types=FALSE) %>%
...
earn_sex19 <- earnings_df19 %>%
  filter(gender!="Total") %>%
  arrange(desc(gender)) %>%
  mutate(W=earn, G=if_else(gender=="Male",0,1))
g_earnings_gender <- earn_sex19 %>%
  select(W, G) %>%
...
```{r allt-annat-lika-livslangd-inkomst-gender, echo=FALSE, out.width="71%", fig.cap="Life expectancy, earnings and gender"}
library("patchwork")
life19 <- read_csv("data/life-expectancy-of-women-vs-life-expectancy-of-men.csv", show_col_types=FALSE) %>%
  select(year=Year, country=Entity,
  ...
  mutate(gender=case_when(name=="life_exp_male"~"Male", name=="life_exp_female"~"Female")) %>%
  filter(year==2020) %>%
  select(-name, -year) %>% rename(life_exp=value)
mydf19 <- life19 %>% right_join(earn_sex19) %>%
  select(country, gender, life_exp, earn, G) %>%
  mutate(L=life_exp, I=earn)
est19 <- lm(L~I+G, data=mydf19 %>% select(L,I,G))
g_life_inc_gender_1 <- mydf19 %>%
  ggplot(aes(y=L, x=I)) +
  geom_point(aes(shape=as.factor(G)), size=2.5, show.legend=FALSE) +
  geom_smooth(method="lm", se=FALSE, linetype="dashed") +
  labs(y="Life expectancy", x="Earnings, USD PPP") +
  annotate("text", x=c(22,16), y=c(78,85), label=c("Men","Women"), size=4.5) +
  theme(text=element_text(size=12))
xmin_male19 <- min(mydf19 %>% filter(G==0) %>% pull(I))
xmax_male19 <- max(mydf19 %>% filter(G==0) %>% pull(I))
xmin_women19 <- min(mydf19 %>% filter(G==1) %>% pull(I))
xmax_women19 <- max(mydf19 %>% filter(G==1) %>% pull(I))
g_life_inc_gender_2 <- mydf19 %>%
  ggplot(aes(y=L, x=I)) +
  geom_point(aes(shape=as.factor(G)), size=2.5, show.legend=FALSE) +
  geom_function(fun=function(x) est19$coefficients[1]+est19$coefficients[2]*x,
                linetype="dashed", linewidth=1, xlim=c(xmin_male19, xmax_male19), color="#F8766D") +
  geom_function(fun=function(x) est19$coefficients[1]+est19$coefficients[2]*x+est19$coefficients[3],
                linetype="dashed", linewidth=1, xlim=c(xmin_women19, xmax_women19), color="#00BFC4") +
  labs(y="Life expectancy", x="Earnings, USD PPP") +
  annotate("text", x=c(22,16), y=c(78,85), label=c("Men","Women"), size=4.5) +
  theme(text=element_text(size=12))
...
```{r livslangd-inkomst-kon, echo=FALSE, out.width="71%", fig.cap="Life expectancy and earnings for women and men"}
intdf19 <- mydf19 %>%
  mutate(GI=G*I) %>%
  select(country, L, I, G, GI)
g_194_life_earning_gender_ols <- intdf19 %>%
  ggplot(aes(x=I, y=L, group=as.factor(G), shape=as.factor(G))) +
  geom_point(size=3, show.legend=FALSE) +
  geom_smooth(method="lm", se=FALSE, linetype=2, linewidth=.7) +
  labs(x="Earnings", y="Life expectancy") +
  theme(text=element_text(size=12)) +
  annotate("text", x=c(18,21), y=c(83,80), size=4.5, label=c("Women","Men"))
...
$$
\begin{equation}
R^{2}=1-\frac{\sum\left(y_{i}-\left(\hat{b}_{0}+\hat{b}_{1}x_{1}+...+\hat{b}_{k}x_{k}+\hat{b}_{k+1}x_{k+1}+\hat{e}_{i}\right)\right)^{2}}{\sum\left(y_{i}-\bar{y}\right)^{2}}
\end{equation}
$$
...
```{r 23_least_squares_multiple_variables_ex_5_pdf, echo=FALSE, results='asis'}
if (knitr::is_latex_output()) {
  ex_items <- list(
    list(
      q = "A multiple regression gives \\(\\hat{Y}=2+3X_{1}+1.5X_{2}\\) , where \\(X_{1}\\) is years of education and \\(X_{2}\\) is years of experience:\n\n(a) What is the predicted (Y) when (X_{1}=4) and (X_{2}=2) ?\n\n(b) Interpret the coefficient (hat{b}_{1}=3) holding (X_{2}) constant.\n\n(c) Interpret the coefficient (hat{b}_{2}=1.5) holding (X_{1}) constant.",
      a = "(a)\\begin_inset Formula \\(\\hat{Y}=2+12+3=17\\) , (b) one additional year of education is associated with 3 units higher \\(Y\\) on average, holding experience constant, (c) one additional year of experience is associated with 1.5 units higher \\(Y\\) on average, holding education constant.\n\n"
    ),
    list(
      q = "A regression model includes a gender dummy variable \\(D\\) (1 = female, 0 = male): \\(\\hat{Y}=500+50X+100D\\) .\n\n(a) What is the predicted income for a male with (X=10) ?\n\n(b) What is the predicted income for a female with (X=10) ?\n\n(c) What does the coefficient 100 represent?",
      a = "(a)\\begin_inset Formula \\(\\hat{Y}=1000\\) , (b)\\begin_inset Formula \\(\\hat{Y}=1100\\) , (c) the average income difference between females and males, holding \\(X\\) constant.\n\n"
    ),
    list(
      q = "A multiple regression gives \\(\\hat{Y}=10+2X_{1}+3X_{2}\\) .\n\n(a) What is the ceteris paribus effect on (hat{Y}) of a one-unit increase in (X_{1}) ?\n\n(b) If (X_{1}) increases by 2 and (X_{2}) stays constant, what is the change in (hat{Y}) ?\n\n(c) Why is the ceteris paribus condition important in multiple regression?",
      a = "(a) 2, (b) \\(\\Delta\\hat{Y}=4\\) , (c) it allows us to isolate the effect of one variable while controlling for others.\n\n"
    ),
    list(
      q = "Given \\(\\hat{Y}=1+2X_{1}+3X_{2}\\) , calculate the predicted value for:\n\n(a) (X_{1}=1,X_{2}=2) .\n\n(b) (X_{1}=3,X_{2}=1) .\n\n(c) (X_{1}=0,X_{2}=4) .",
      a = "(a)\\begin_inset Formula \\(\\hat{Y}=9\\) , (b)\\begin_inset Formula \\(\\hat{Y}=14\\) , (c)\\begin_inset Formula \\(\\hat{Y}=13\\) .\n\n"
    ),
    list(
      q = "A regression with a country dummy \\(D_{S}\\) (1 = Sweden, 0 = other) gives \\(\\hat{Y}=100+5X+20D_{S}\\) .\n\n(a) What is the predicted (Y) for a non-Swedish country with (X=10) ?\n\n(b) What is the predicted (Y) for Sweden with (X=10) ?\n\n(c) What does the coefficient 20 represent?",
      a = "(a)\\begin_inset Formula \\(\\hat{Y}=150\\) , (b)\\begin_inset Formula \\(\\hat{Y}=170\\) , (c) the average difference in \\(Y\\) for Sweden compared to other countries, holding \\(X\\) constant.\n\n"
    ),
    list(
      q = "Discuss model specification:\n\n(a) What is an underspecified model?\n\n(b) What is an overspecified model?\n\n(c) What problem can underspecification cause?",
      a = "(a) a model missing one or more relevant explanatory variables, (b) a model with irrelevant variables included, (c) omitted variable bias — the coefficients may be incorrect.\n\n"
    ),
    list(
      q = "Model A has \\(R^{2}=0.65\\) with 1 explanatory variable. Model B has \\(R^{2}=0.70\\) with 4 variables and adjusted \\(R^{2}=0.66\\) .\n\n(a) Which model has higher (R^{2}) ?\n\n(b) Why use adjusted (R^{2}) when comparing models?\n\n(c) Which model is preferred according to adjusted (R^{2}) ?",
      a = "(a) model B, (b) adjusted \\(R^{2}\\) penalizes for adding variables that do not improve fit, (c) model B (adjusted \\(R^{2}\\) 0.66 > 0.65).\n\n"
    ),
    list(
      q = "In the matrix form of OLS, \\(Y=XB+V\\) :\n\n(a) What does the matrix (X) contain?\n\n(b) What does the vector (B) contain?\n\n(c) Write the OLS estimator (hat{B}) .",
      a = "(a) a column of ones and the explanatory variables, (b) the regression coefficients \\(\\hat{a},\\hat{b}_{1},\\hat{b}_{2},\\ldots\\) , (c)\\begin_inset Formula \\(\\hat{B}=(X'X)^{-1}X'Y\\) .\n\n"
    ),
    list(
      q = "A regression includes an interaction term: \\(\\hat{Y}=5+2X_{1}+3X_{2}+1.5X_{1}X_{2}\\) .\n\n(a) What is the effect of (X_{1}) on (hat{Y}) when (X_{2}=0) ?\n\n(b) What is the effect of (X_{1}) on (hat{Y}) when (X_{2}=2) ?\n\n(c) What does the interaction term (1.5X_{1}X_{2}) capture?",
      a = "(a) 2, (b)\\begin_inset Formula \\(2+1.5\\times2=5\\) , (c) that the effect of \\(X_{1}\\) on \\(Y\\) depends on the value of \\(X_{2}\\) .\n\n"
    ),
    list(
      q = "Given \\(n=100\\) , \\(k=4\\) explanatory variables, \\(TSS=500\\) , \\(SSE=100\\) :\n\n(a) Calculate (R^{2}) .\n\n(b) Calculate adjusted (bar{R}^{2}=1-frac{SSE/(n-k-1)}{TSS/(n-1)}) .\n\n(c) Is (bar{R}^{2}) higher or lower than (R^{2}) ?",
      a = "(a)\\begin_inset Formula \\(R^{2}=0.8\\) , (b)\\begin_inset Formula \\(\\bar{R}^{2}=1-\\frac{100/95}{500/99}\\approx0.791\\) , (c) \\(\\bar{R}^{2}\\) is lower than \\(R^{2}\\) .\n\n"
    ),
    list(
      q = "A regression of income \\(Y\\) on education \\(X_{1}\\) and age \\(X_{2}\\) gives: \\(\\hat{Y}=100+80X_{1}+10X_{2}\\) .\n\n(a) What is the predicted income for a person with (X_{1}=10) years of education and (X_{2}=30) years of age?\n\n(b) By how much does income increase per additional year of education, holding age constant?\n\n(c) By how much does income increase per additional year of age, holding education constant?",
      a = "(a)\\begin_inset Formula \\(\\hat{Y}=100+800+300=1200\\) , (b) 80, (c) 10.\n\n"
    ),
    list(
      q = "A researcher adds a variable \\(X_{3}\\) to a model and observes that \\(R^{2}\\) increases from 0.72 to 0.74 but adjusted \\(R^{2}\\) decreases from 0.70 to 0.69.\n\n(a) Why does (R^{2}) always increase (or stay the same) when a variable is added?\n\n(b) What does the decrease in adjusted (R^{2}) suggest about (X_{3}) ?\n\n(c) Should the researcher keep (X_{3}) in the model?",
      a = "(a) adding variables always reduces SSE or leaves it unchanged, which mechanically increases \\(R^{2}\\) , (b) \\(X_{3}\\) does not contribute enough to justify its inclusion, (c) no, based on adjusted \\(R^{2}\\) .\n\n"
    )
  )
