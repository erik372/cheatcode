...
```{r gdp-and-happiness-ols-illustration-1, echo=FALSE, out.width="71%", fig.cap="GDP and happiness"}
library("tidyverse")
library("ggrepel")
library("gghighlight")
my_select_countries <- c("Sweden","Brazil","Congo","France","Jamaica","Haiti")
df17 <- read_delim("data/gdp-vs-happiness2.csv", delim=";", show_col_types=FALSE) %>%
  filter(Year==2017) %>%
...
```{r tab-data-for-ols-1, echo=FALSE, out.width="71%", fig.cap="Four observations for $x$ and $y$", fig.width=5.5, fig.height=3.5}
library("patchwork")
library("grid")
t2_17 <- tibble(obs=1:4, x=c(3,4,6,7), y=c(3,2,5,4),
                label=paste0("(", c(3,4,6,7), ",", c(3,2,5,4), ")"))
g_olspoints_17 <- t2_17 %>%
  ggplot(aes(x,y)) + geom_point() +
  geom_text_repel(aes(label=label), size=3.5, position="dodge") +
  theme(axis.title.y=element_text(angle=0), text=element_text(size=10)) +
  annotate('text', x=3, y=4.1, hjust=0, size=3.5, label="This is\nobservation 1") +
  geom_curve(aes(x=3.5, y=3.8, xend=3.2, yend=3.2),
             curvature=-.3, arrow=arrow(length=unit(.2,'cm')))
tbl_cells <- data.frame(col=rep(1:3,5), row=rep(0:4,each=3))
tbl_labels <- data.frame(
  col=rep(1:3,5), row=rep(0:4,each=3),
  label=c("bold('Obs. i')","bold(X[i])","bold(Y[i])",
          "1","3","3","2","4","2","3","6","5","4","7","4"),
  is_header=rep(c(TRUE,FALSE,FALSE,FALSE,FALSE),each=3)
)
g_tbl_17 <- ggplot(tbl_cells, aes(x=col, y=-row)) +
  geom_tile(fill="white", color="gray50", linewidth=0.4) +
  geom_text(data=subset(tbl_labels, is_header),
            aes(x=col, y=-row, label=label), parse=TRUE, size=3.2) +
  geom_text(data=subset(tbl_labels, !is_header),
            aes(x=col, y=-row, label=label), size=3.2) +
  theme_void()
wrap_elements(g_tbl_17) + g_olspoints_17 + plot_layout(widths=c(3,7))
```
...
```{r ols-1, echo=FALSE, out.width="71%", fig.cap="A line drawn using the least squares method"}
library("latex2exp")
lm1_17 <- lm(y~x, t2_17)
t2_17 <- t2_17 %>% mutate(yhat=fitted.values(lm1_17))
g_ols <- t2_17 %>% ggplot(aes(x,y)) +
  geom_point() +
  geom_smooth(method="lm", se=FALSE) +
  geom_segment(aes(xend=x, yend=yhat), alpha=.45, linetype='dashed') +
  geom_text_repel(aes(label=label), size=4, position="dodge") +
  annotate('text', x=5.5, y=3, size=4, hjust=0, label="The line is drawn using\nthe equation") +
  annotate('text', x=5.5, y=2.7, size=4, hjust=0, label=TeX("$\\hat{y}_i = \\hat{a} + \\hat{b}x_i$")) +
  geom_curve(aes(x=5.45, y=3, xend=4.7, yend=3.3),
             curvature=-.3, arrow=arrow(length=unit(.2,'cm'))) +
  annotate('text', x=5.2, y=4.75, size=4.5, hjust=1, label=TeX("$\\hat{v}_3 = 5 - 4 = 1$")) +
...
```{r tre-rata-linjer, echo=FALSE, out.width="71%", fig.cap="Three straight lines"}
g_falseols <- t2_17 %>% ggplot(aes(x,y)) + geom_point() +
  geom_text_repel(aes(label=label), size=4, position="dodge") +
  geom_smooth(method="lm", se=FALSE, linewidth=.2, linetype='dashed') +
  geom_hline(yintercept=3.5, color='black', linewidth=.2, linetype='dashed') +
  stat_function(fun=function(x) 9/4 + .25*x, linewidth=.2, linetype='dashed', color="#F8766D") +
  annotate('label', x=6, y=4.1, label="A", size=5) +
  annotate('label', x=6, y=3.7, label="C", size=5) +
  annotate('label', x=3.5, y=3.5, label="B", size=5) +
  theme(axis.title.y=element_text(angle=0), text=element_text(size=12))
g_falseols
```
...
```{r anscombe, echo=FALSE, out.width="71%", fig.cap="Four examples of linear covariation, from [(Anscombe, 1973)](https://www.jstor.org/stable/2682899)"}
library("patchwork")
anscombe <- datasets::anscombe
g4_theme <- theme(axis.title.y=element_text(angle=0), text=element_text(size=11),
                  axis.ticks=element_blank(), axis.text=element_blank())
g_anscombe1 <- anscombe %>% mutate(ypred=predict(lm(y1~x1)), xi=x1) %>%
  ggplot(aes(x1,y1)) + geom_point(size=1.2) + geom_smooth(method='lm',se=FALSE,linewidth=.5) +
  geom_segment(aes(xend=xi,yend=ypred),alpha=.25,linetype='dashed') + g4_theme
g_anscombe2 <- anscombe %>% mutate(ypred=predict(lm(y2~x2)), xi=x2) %>%
  ggplot(aes(x2,y2)) + geom_point(size=1.2) + geom_smooth(method='lm',se=FALSE,linewidth=.5) +
  geom_segment(aes(xend=xi,yend=ypred),alpha=.25,linetype='dashed') + g4_theme
g_anscombe3 <- anscombe %>% mutate(ypred=predict(lm(y3~x3)), xi=x3) %>%
  ggplot(aes(x3,y3)) + geom_point(size=1.2) + geom_smooth(method='lm',se=FALSE,linewidth=.5) +
  geom_segment(aes(xend=xi,yend=ypred),alpha=.25,linetype='dashed') + g4_theme
g_anscombe4 <- anscombe %>% mutate(ypred=predict(lm(y4~x4)), xi=x4) %>%
  ggplot(aes(x4,y4)) + geom_point(size=1.2) + geom_smooth(method='lm',se=FALSE,linewidth=.5) +
  geom_segment(aes(xend=xi,yend=ypred),alpha=.25,linetype='dashed') + g4_theme
...
```{r weird-ols-2, echo=FALSE, out.width="71%", fig.cap="Regression lines with zero slope"}
g_weirdols_ex3 <- tibble(x=0:11, y=if_else(0:11 <= 5, .945+(0:11), -7+(0:11))) %>%
...
g_weirdols_ex4 <- tibble(x=0:10, y=10*(0:10)-(0:10)^2+runif(1)) %>%
  mutate(ypred=predict(lm(y~x))) %>%
  ggplot(aes(x=x,y=y)) + geom_point(size=2) +
  geom_smooth(method='lm', se=FALSE, linewidth=.5) +
  geom_segment(aes(xend=x,yend=ypred), alpha=.3, linetype='dashed') +
...
```{r ols-ex-2, echo=FALSE, out.width="71%", fig.cap="Four observations and a regression line"}
g_ols_ex2 <- tibble(Z=c(1,4,0,1), K=c(0,0,4,4),
                    label=paste0("(", c(0,0,4,4), ",", c(1,4,0,1), ")")) %>%
  mutate(zpred=predict(lm(Z~K))) %>%
  ggplot(aes(x=K,y=Z)) + geom_point() +
  geom_text_repel(aes(label=label), size=4, nudge_x=.1) +
  geom_smooth(method='lm', se=FALSE, linewidth=.5) +
  geom_segment(aes(xend=K,yend=zpred), alpha=.3, linetype='dashed') +
  theme(axis.title.y=element_text(angle=0), text=element_text(size=12)) +
  xlim(0,5) + ylim(0,5) +
  annotate('text', x=2, y=3, size=4, hjust=0, label="The line is drawn with the equation") +
  annotate('text', x=2, y=2.6, size=4, hjust=0, label=TeX("$\\hat{Z}_i = \\hat{\\alpha} + \\hat{\\beta} K_i$"))
g_ols_ex2
```
...
```{r differens-mellan-y-ybar-yhat, echo=FALSE, out.width="71%", fig.cap="Difference between $Y_{i},\\bar{Y}_{i}$ and $\\hat{Y}_{i}$"}
library("ggbrace")
t2_rss <- tibble(x=c(3,4,6,7), y=c(3,2,5,4)) %>%
  mutate(yhat=fitted.values(lm(y~x)))
g_rssesstss <- t2_rss %>% ggplot(aes(x,y)) +
  geom_point() +
  geom_smooth(method="lm", se=FALSE, linewidth=.5) +
  geom_segment(aes(xend=x,yend=yhat), alpha=.45, linetype='dotted') +
  geom_hline(yintercept=3.5, linetype="longdash") +
  annotate("label", y=c(3.5,2.6,2), x=c(3,3.2,3.8), size=3.5,
           label=c(TeX("\\bar{y}"), TeX("\\hat{y}"), TeX("$y_i$"))) +
  ...
  annotate("text", x=c(3.5,4.5,5.8), y=c(3.25,2.5,2.75), size=3.5,
           label=c(TeX("$\\hat{y} - \\bar{y}$"), TeX("$y_i - \\hat{y}$"), TeX("$y_i - \\bar{y}$"))) +
  theme(axis.title.y=element_text(angle=0), text=element_text(size=11))
g_rssesstss
```
...
```{r 21_least_squares_ex_3_pdf, echo=FALSE, results='asis'}
if (knitr::is_latex_output()) {
  ex_items <- list(
    list(
      q = "Given \\(X=\\{0,1,2,3\\}\\) and \\(Y=\\{1,3,4,4\\}\\) :\n\n(a) Calculate (bar{X}) and (bar{Y}) .\n\n(b) Calculate (sum_{i}(X_{i}-bar{X})^{2}) .\n\n(c) Calculate (sum_{i}(X_{i}-bar{X})(Y_{i}-bar{Y})) .",
      a = "(a) \\(\\bar{X}=1.5,\\bar{Y}=3\\) , (b) \\(\\sum(X_{i}-\\bar{X})^{2}=5\\) , (c) \\(\\sum(X_{i}-\\bar{X})(Y_{i}-\\bar{Y})=5\\) .\n\n"
    ),
    list(
      q = "Continued from exercise 1, using \\(\\hat{b}=\\frac{\\sum(X_{i}-\\bar{X})(Y_{i}-\\bar{Y})}{\\sum(X_{i}-\\bar{X})^{2}}\\) and \\(\\hat{a}=\\bar{Y}-\\hat{b}\\bar{X}\\) :\n\n(a) Calculate the slope (hat{b}) .\n\n(b) Calculate the intercept (hat{a}) .\n\n(c) Write the estimated regression equation (hat{Y}_{i}=hat{a}+hat{b}X_{i}) .",
      a = "(a) \\(\\hat{b}=1\\) , (b) \\(\\hat{a}=1.5\\) , (c) \\(\\hat{Y}=1.5+X\\) .\n\n"
    ),
    list(
      q = "Given the regression equation \\(\\hat{Y}=1.5+X\\) :\n\n(a) What is the fitted value (hat{Y}) when (X=2) ?\n\n(b) If (Y=4) when (X=2) , what is the residual (hat{V}=Y-hat{Y}) ?\n\n(c) What property do OLS residuals always satisfy regarding their sum?",
      a = "(a) \\(\\hat{Y}=3.5\\) , (b) \\(\\hat{V}=0.5\\) , (c) OLS residuals always sum to zero.\n\n"
    ),
    list(
      q = "Using \\(X=\\{0,1,2,3\\}\\) , \\(Y=\\{1,3,4,4\\}\\) and \\(\\hat{Y}=1.5+X\\) :\n\n(a) Calculate (SSE=sum(Y_{i}-hat{Y}_{i})^{2}) .\n\n(b) Calculate (TSS=sum(Y_{i}-bar{Y})^{2}) .\n\n(c) Calculate (R^{2}=1-frac{SSE}{TSS}) .",
      a = "(a) \\(SSE=1\\) , (b) \\(TSS=6\\) , (c) \\(R^{2}=5/6\\approx0.83\\) .\n\n"
    ),
    list(
      q = "Given \\(cov(X,Y)=12\\) and \\(var(X)=4\\) :\n\n(a) Calculate (hat{b}=frac{cov(X,Y)}{var(X)}) .\n\n(b) If (bar{X}=3) and (bar{Y}=10) , calculate (hat{a}) .\n\n(c) Write the regression equation.",
      a = "(a) \\(\\hat{b}=3\\) , (b) \\(\\hat{a}=1\\) , (c) \\(\\hat{Y}=1+3X\\) .\n\n"
    ),
    list(
      q = "Interpret the regression result \\(\\hat{Y}=500+50X\\) , where \\(X\\) is years of education and \\(Y\\) is monthly income in USD:\n\n(a) Interpret the intercept (hat{a}=500) .\n\n(b) Interpret the slope (hat{b}=50) .\n\n(c) What does (R^{2}=0.6) tell us?",
      a = "(a) predicted income with 0 years of education is 500 USD, (b) each additional year of education is associated with 50 USD higher income on average, (c) 60% of the variation in income is explained by the model.\n\n"
    ),
    list(
      q = "A regression of happiness \\(Y\\) on GDP per capita \\(X\\) gives \\(\\hat{Y}=3+0.5X\\) .\n\n(a) What is the predicted happiness when (X=4) ?\n\n(b) If observed happiness is 6 when (X=4) , what is the residual?\n\n(c) Does the positive slope mean that GDP causes happiness?",
      a = "(a) \\(\\hat{Y}=5\\) , (b) \\(\\hat{V}=1\\) , (c) no — correlation does not imply causation.\n\n"
    ),
    list(
      q = "For the regression \\(\\hat{Y}=\\hat{a}+\\hat{b}X\\) , describe what happens when:\n\n(a) (hat{b}=0) .\n\n(b) (hat{b}>0) .\n\n(c) (hat{b}<0) .",
      a = "(a) no linear covariation between \\(X\\) and \\(Y\\) , (b) positive covariation, (c) negative covariation.\n\n"
    ),
    list(
      q = "For two observations \\(X=\\{1,3\\}\\) and \\(Y=\\{2,8\\}\\) :\n\n(a) Calculate (bar{X}) and (bar{Y}) .\n\n(b) Calculate (hat{b}) .\n\n(c) Calculate (hat{a}) .",
      a = "(a) \\(\\bar{X}=2,\\bar{Y}=5\\) , (b) \\(\\hat{b}=3\\) , (c) \\(\\hat{a}=-1\\) .\n\n"
    ),
    list(
      q = "Given \\(TSS=20\\) and \\(SSE=5\\) :\n\n(a) Calculate (R^{2}) .\n\n(b) Calculate (SSR=TSS-SSE) .\n\n(c) Interpret the value of (R^{2}) .",
      a = "(a) \\(R^{2}=0.75\\) , (b) \\(SSR=15\\) , (c) 75% of the variation in \\(Y\\) is explained by the model.\n\n"
    ),
    list(
      q = "Given the regression \\(\\hat{Y}=2+0.5X\\) , predict \\(Y\\) for:\n\n(a) (X=10) .\n\n(b) (X=0) .\n\n(c) (X=6) .",
      a = "(a) \\(\\hat{Y}=7\\) , (b) \\(\\hat{Y}=2\\) , (c) \\(\\hat{Y}=5\\) .\n\n"
    ),
    list(
      q = "A set of residuals from an OLS regression is \\(\\hat{V}=\\{-1,2,-1,0\\}\\) .\n\n(a) Do the residuals sum to zero?\n\n(b) Calculate (SSE=sumhat{V}_{i}^{2}) .\n\n(c) If (TSS=12) , calculate (R^{2}) .",
      a = "(a) yes, sum \\(=0\\) , (b) \\(SSE=6\\) , (c) \\(R^{2}=0.5\\) .\n\n"
    )
  )
...
(function(){
var ex=[
  {q:'<p>Given \\(X=\\{0,1,2,3\\}\\) and \\(Y=\\{1,3,4,4\\}\\) :</p><ol type="a"><li>Calculate \\(\\bar{X}\\) and \\(\\bar{Y}\\) .</li><li>Calculate \\(\\sum_{i}(X_{i}-\\bar{X})^{2}\\) .</li><li>Calculate \\(\\sum_{i}(X_{i}-\\bar{X})(Y_{i}-\\bar{Y})\\) .</li></ol>',a:'<p>(a) \\(\\bar{X}=1.5,\\bar{Y}=3\\) , (b) \\(\\sum(X_{i}-\\bar{X})^{2}=5\\) , (c) \\(\\sum(X_{i}-\\bar{X})(Y_{i}-\\bar{Y})=5\\) .</p>'},
  {q:'<p>Continued from exercise 1, using \\(\\hat{b}=\\frac{\\sum(X_{i}-\\bar{X})(Y_{i}-\\bar{Y})}{\\sum(X_{i}-\\bar{X})^{2}}\\) and \\(\\hat{a}=\\bar{Y}-\\hat{b}\\bar{X}\\) :</p><ol type="a"><li>Calculate the slope \\(\\hat{b}\\) .</li><li>Calculate the intercept \\(\\hat{a}\\) .</li><li>Write the estimated regression equation \\(\\hat{Y}_{i}=\\hat{a}+\\hat{b}X_{i}\\) .</li></ol>',a:'<p>(a) \\(\\hat{b}=1\\) , (b) \\(\\hat{a}=1.5\\) , (c) \\(\\hat{Y}=1.5+X\\) .</p>'},
  {q:'<p>Given the regression equation \\(\\hat{Y}=1.5+X\\) :</p><ol type="a"><li>What is the fitted value \\(\\hat{Y}\\) when \\(X=2\\) ?</li><li>If \\(Y=4\\) when \\(X=2\\) , what is the residual \\(\\hat{V}=Y-\\hat{Y}\\) ?</li><li>What property do OLS residuals always satisfy regarding their sum?</li></ol>',a:'<p>(a) \\(\\hat{Y}=3.5\\) , (b) \\(\\hat{V}=0.5\\) , (c) OLS residuals always sum to zero.</p>'},
  {q:'<p>Using \\(X=\\{0,1,2,3\\}\\) , \\(Y=\\{1,3,4,4\\}\\) and \\(\\hat{Y}=1.5+X\\) :</p><ol type="a"><li>Calculate \\(SSE=\\sum(Y_{i}-\\hat{Y}_{i})^{2}\\) .</li><li>Calculate \\(TSS=\\sum(Y_{i}-\\bar{Y})^{2}\\) .</li><li>Calculate \\(R^{2}=1-\\frac{SSE}{TSS}\\) .</li></ol>',a:'<p>(a) \\(SSE=1\\) , (b) \\(TSS=6\\) , (c) \\(R^{2}=5/6\\approx0.83\\) .</p>'},
  {q:'<p>Given \\(cov(X,Y)=12\\) and \\(var(X)=4\\) :</p><ol type="a"><li>Calculate \\(\\hat{b}=\\frac{cov(X,Y)}{var(X)}\\) .</li><li>If \\(\\bar{X}=3\\) and \\(\\bar{Y}=10\\) , calculate \\(\\hat{a}\\) .</li><li>Write the regression equation.</li></ol>',a:'<p>(a) \\(\\hat{b}=3\\) , (b) \\(\\hat{a}=1\\) , (c) \\(\\hat{Y}=1+3X\\) .</p>'},
  ...
  {q:'<p>For two observations \\(X=\\{1,3\\}\\) and \\(Y=\\{2,8\\}\\) :</p><ol type="a"><li>Calculate \\(\\bar{X}\\) and \\(\\bar{Y}\\) .</li><li>Calculate \\(\\hat{b}\\) .</li><li>Calculate \\(\\hat{a}\\) .</li></ol>',a:'<p>(a) \\(\\bar{X}=2,\\bar{Y}=5\\) , (b) \\(\\hat{b}=3\\) , (c) \\(\\hat{a}=-1\\) .</p>'},
  {q:'<p>Given \\(TSS=20\\) and \\(SSE=5\\) :</p><ol type="a"><li>Calculate \\(R^{2}\\) .</li><li>Calculate \\(SSR=TSS-SSE\\) .</li><li>Interpret the value of \\(R^{2}\\) .</li></ol>',a:'<p>(a) \\(R^{2}=0.75\\) , (b) \\(SSR=15\\) , (c) 75% of the variation in \\(Y\\) is explained by the model.</p>'},
  {q:'<p>Given the regression \\(\\hat{Y}=2+0.5X\\) , predict \\(Y\\) for:</p><ol type="a"><li>\\(X=10\\) .</li><li>\\(X=0\\) .</li><li>\\(X=6\\) .</li></ol>',a:'<p>(a) \\(\\hat{Y}=7\\) , (b) \\(\\hat{Y}=2\\) , (c) \\(\\hat{Y}=5\\) .</p>'},
  {q:'<p>A set of residuals from an OLS regression is \\(\\hat{V}=\\{-1,2,-1,0\\}\\) .</p><ol type="a"><li>Do the residuals sum to zero?</li><li>Calculate \\(SSE=\\sum\\hat{V}_{i}^{2}\\) .</li><li>If \\(TSS=12\\) , calculate \\(R^{2}\\) .</li></ol>',a:'<p>(a) yes, sum \\(=0\\) , (b) \\(SSE=6\\) , (c) \\(R^{2}=0.5\\) .</p>'}
];
