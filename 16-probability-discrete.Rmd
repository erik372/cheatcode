...
```{r tva-tarningar, echo=FALSE, fig.cap="Number of cases and result sum for two dice", fig.width=8, fig.height=3.5}
library(ggplot2)
library(patchwork)

# Left panel: 6x6 combinations table
dice_df <- expand.grid(Red=1:6, Black=1:6)
dice_df$Sum <- dice_df$Red + dice_df$Black

g_tbl <- ggplot(dice_df, aes(x=Black, y=Red, label=Sum)) +
  geom_tile(fill="white", colour="grey50", linewidth=0.4) +
  geom_text(size=3.5) +
  scale_x_continuous(breaks=1:6, position="top") +
  scale_y_reverse(breaks=1:6) +
  labs(x="Black dice", y="Red dice") +
  theme_minimal() +
...
# Right panel: bar chart (matches original g_2dices)
df_bar <- data.frame(x=2:12, y=c(1,2,3,4,5,6,5,4,3,2,1))
g_bar <- ggplot(df_bar, aes(x=x, y=y)) +
  geom_col(width=0.5) +
  scale_x_continuous(breaks=2:12) +
  scale_y_continuous(breaks=1:6) +
  labs(x="Points", y="Number of cases") +
  theme(text=element_text(size=10))

g_tbl + g_bar + plot_layout(widths=c(1.4, 1))
```
...
```{r balls-table, echo=FALSE}
df_balls <- data.frame(
  Color    = c("Red", "Yellow", "Sum"),
  Filled   = c(40, 10, 50),
  Hollow   = c(40, 10, 50),
  Sum      = c(80, 20, 100)
)
knitr::kable(df_balls, caption="Three types of balls", booktabs=TRUE,
             col.names=c("", "Filled", "Hollow", "Sum"),
             align=c("l","c","c","c"))
```
...
```{r dicemeans, echo=FALSE, fig.cap="Law of large numbers: simulated means from a die (values 1–6) for increasing sample sizes. The dashed line marks the expected value 3.5.", fig.width=7, fig.height=3.5}
library(ggplot2)
library(patchwork)
set.seed(123)

diceroll <- function(x) mean(sample(1:6, size=x, replace=TRUE))
rolls <- 1000
dice_df <- data.frame(r=1:rolls, sm=sapply(1:rolls, diceroll))

gh <- geom_hline(yintercept=3.5, linetype="dashed", alpha=0.33)
gl <- labs(x="Number of dice throws\nper mean value", y="Mean")

g_10dices <- ggplot(dice_df[1:10,], aes(x=r, y=sm)) +
  geom_line() + geom_point() + gh + gl +
  scale_x_continuous(breaks=1:10) +
  scale_y_continuous(breaks=1:6, limits=c(1,6)) +
  theme(text=element_text(size=10))

g_1000dices <- ggplot(dice_df, aes(x=r, y=sm)) +
  geom_line(linewidth=0.1) + gh + gl +
  scale_x_continuous(breaks=seq(0,1000,100)) +
  scale_y_continuous(breaks=1:6, limits=c(1,6)) +
  theme(text=element_text(size=10), axis.text.x=element_text(angle=90, vjust=0.5))
...
```{r dice-pmf-cdf, echo=FALSE, fig.cap="Probability mass function (top) and cumulative distribution function (bottom) for a uniform discrete probability distribution that can assume the integers 1 to 6.", fig.width=6.25, fig.height=6.25}
library(ggplot2)
library(patchwork)

tb1 <- data.frame(x=1:6, pdf=1/6, cdf=cumsum(rep(1/6,6)), x1=2:7)

p_pmf <- ggplot(tb1, aes(x=x, y=pdf)) +
  geom_col(width=0.3) +
  geom_segment(x=0, xend=7, y=1/6, yend=1/6, linetype="dashed") +
  annotate("text", x=0, y=0.18, size=2.5, hjust=0, label="1/6 probability") +
  scale_x_continuous(breaks=1:6, limits=c(0,7)) +
  scale_y_continuous(breaks=seq(0,.2,.05), limits=c(0,.2)) +
  labs(x="Values for variable X: 1 to 6", y="Probability",
       title="Probability mass function") +
  theme(text=element_text(size=14))

p_cdf <- ggplot(tb1, aes(y=cdf, x=x)) +
  geom_point() +
  ...
  annotate("text", x=0, y=1.05, size=2.5, hjust=0,
           label="100% cumulative probability.") +
  scale_x_continuous(breaks=1:6, limits=c(0,7)) +
  scale_y_continuous(breaks=seq(0,1,.2), limits=c(0,1.1)) +
  labs(x="Values for variable X", y="Cumulative\nprobability",
...
```{r binomial-pmf-cdf, echo=FALSE, fig.cap="Probability mass function (top) and cumulative distribution function (bottom) for a binomial distribution with \\(n=10\\) and \\(p=0.5\\).", fig.width=6.25, fig.height=6.25}
library(ggplot2)
library(patchwork)

n <- 10; p <- 0.5
tb_binom <- data.frame(x=0:n, pdf=dbinom(0:n,n,p), cdf=pbinom(0:n,n,p), x1=1:(n+1))

p_pmf <- ggplot(tb_binom, aes(x=x, y=pdf)) +
  geom_col(width=0.3) +
  annotate("text", x=0, y=0.18, size=2.5, hjust=0,
           label="Probability that exactly\n3 of 10 tosses\nresult in tails") +
  geom_segment(aes(x=0.5, xend=2.4, y=0.15, yend=0.1),
               arrow=arrow(length=unit(0.1,'cm'))) +
  scale_x_continuous(breaks=0:10) +
  labs(x="Number of tosses resulting in tails", y="Probability",
       title="Probability mass function") +
  theme(text=element_text(size=14))

p_cdf <- ggplot(tb_binom, aes(y=cdf, x=x)) +
  geom_point(size=1) +
  geom_segment(aes(x=x, xend=x1, yend=cdf), linewidth=0.5) +
  geom_segment(x=-0.5, xend=-0.2, y=0, yend=0, linewidth=0.5) +
  geom_hline(yintercept=1, linewidth=0.3, alpha=0.2) +
  annotate("text", x=0, y=1.05, hjust=0, size=2.5,
           label="100% cumulative probability.") +
  annotate("segment", x=3, xend=3, y=0, yend=0.4, linetype="dashed") +
  annotate("text", x=0.6, y=0.5, size=2.5, hjust=0,
           label="Probability that 10 tosses\nresult in 0, 1, 2,\nor 3 tails.") +
  scale_x_continuous(breaks=0:10) +
  scale_y_continuous(breaks=seq(0,1,.1)) +
  labs(x="Number of tosses resulting in tails", y="Cumulative\nprobability",
       title="Cumulative distribution function") +
  theme(text=element_text(size=14))
...
```{r bortkiewicz, echo=FALSE, fig.cap="Deaths per cavalry corps per year (Bortkiewicz 1898).", fig.width=6.25, fig.height=4.375}
library(ggplot2)
df_bort <- data.frame(deaths=0:4, n=c(144,91,32,11,2))
lambda_val <- sum(df_bort$deaths * df_bort$n) / sum(df_bort$n)  # 0.7
N <- sum(df_bort$n)  # 280

ggplot(df_bort, aes(x=deaths, y=n)) +
  geom_col() +
  geom_point() +
  stat_function(fun=function(x) exp(-lambda_val) * (lambda_val^x / factorial(x)) * N,
                geom="line", color="#F8766D") +
  annotate("text", x=2, y=70, hjust=0, size=2.5,
           label="In 32 cases 2 soldiers died\nduring the same year\nin one cavalry corps.") +
  annotate("segment", x=2.1, xend=2.1, y=58, yend=41,
               arrow=arrow(length=unit(0.1,'cm'))) +
  scale_y_continuous(
    name="Number of cavalry corps\n(bars)",
    sec.axis=sec_axis(transform=~.*1/N, name="Probability\n(line)")
  ) +
...
```{r 16_probability_discrete_ex_21_pdf, echo=FALSE, results='asis'}
if (knitr::is_latex_output()) {
  ex_items <- list(
    list(
      q = "An urn contains 3 red and 2 blue balls. One ball is drawn at random.\n\n(a) What is (P(text{red}))?\n\n(b) What is (P(text{blue}))?\n\n(c) What is (P(text{red or blue}))?",
      a = "(a) \\(P(\\text{red})=3/5=0.6\\), (b) \\(P(\\text{blue})=2/5=0.4\\), (c) \\(P(\\text{red or blue})=1\\).\n\n"
    ),
    list(
      q = "A random variable \\(X\\) takes values 1, 2, 3 with probabilities 0.3, 0.5, 0.2.\n\n(a) Verify that the probabilities sum to 1.\n\n(b) Calculate (E(X)=sum x_{i}P(X=x_{i})).\n\n(c) Calculate (text{var}(X)=E(X^{2})-[E(X)]^{2}).",
      a = "(a) \\(0.3+0.5+0.2=1\\) ✓, (b) \\(E(X)=1(0.3)+2(0.5)+3(0.2)=1.9\\), (c) \\(E(X^2)=1(0.3)+4(0.5)+9(0.2)=4.1\\), \\(\\text{var}(X)=4.1-1.9^2=0.49\\).\n\n"
    ),
    list(
      q = "Events \\(A\\) and \\(B\\) have \\(P(A)=0.4\\), \\(P(B)=0.3\\), \\(P(A\\cap B)=0.1\\).\n\n(a) Calculate (P(Acup B)).\n\n(b) Are (A) and (B) independent?\n\n(c) Calculate (P(Amid B)).",
      a = "(a) \\(P(A\\cup B)=0.4+0.3-0.1=0.6\\), (b) no: \\(P(A)P(B)=0.12\\neq P(A\\cap B)=0.1\\), (c) \\(P(A\\mid B)=P(A\\cap B)/P(B)=0.1/0.3=1/3\\).\n\n"
    ),
    list(
      q = "A random variable \\(X\\) has \\(E(X)=5\\) and \\(\\text{var}(X)=4\\).\n\n(a) What is (E(2X+3))?\n\n(b) What is (text{var}(2X+3))?\n\n(c) What is the standard deviation (s(2X+3))?",
      a = "(a) \\(E(2X+3)=2\\cdot5+3=13\\), (b) \\(\\text{var}(2X+3)=4\\cdot\\text{var}(X)=16\\), (c) \\(s(2X+3)=\\sqrt{16}=4\\).\n\n"
    ),
    list(
      q = "\\(E(Y\\mid X=1)=8\\) and \\(E(Y\\mid X=0)=3\\), with \\(P(X=1)=0.4\\).\n\n(a) Calculate (E(Y)=E(Ymid X=1)P(X=1)+E(Ymid X=0)P(X=0)).\n\n(b) If (X) represents university education (1=yes, 0=no) and (Y) is income, interpret (E(Ymid X=1)=8).\n\n(c) Does this conditional expectation prove that education causes higher income?",
      a = "(a) \\(E(Y)=8(0.4)+3(0.6)=3.2+1.8=5.0\\), (b) average income among those with university education is 8 units, (c) no — selection bias may explain the difference; education may be correlated with unobserved factors such as family background.\n\n"
    ),
    list(
      q = "A fair die has six equally likely outcomes.\n\n(a) What is (E(X)) for a single roll?\n\n(b) What does the law of large numbers say about the sample mean (bar{X}_{n}) as (nrightarrowinfty)?\n\n(c) Why is this useful for statistical inference?",
      a = "(a) \\(E(X)=3.5\\), (b) \\(\\bar{X}_{n}\\rightarrow\\mu=3.5\\) as \\(n\\rightarrow\\infty\\), (c) large samples give reliable estimates of population parameters — the sample mean converges to the true mean.\n\n"
    ),
    list(
      q = "\\(X\\) is uniformly distributed on \\(\\{1,2,3,4\\}\\) (discrete uniform).\n\n(a) What is the probability mass function (f(x))?\n\n(b) What is (E(X))?\n\n(c) What is (P(2leq Xleq3))?",
      a = "(a) \\(f(x)=1/4\\) for \\(x\\in\\{1,2,3,4\\}\\), (b) \\(E(X)=(1+2+3+4)/4=2.5\\), (c) \\(P(2\\leq X\\leq3)=2/4=0.5\\).\n\n"
    ),
    list(
      q = "A biased coin has \\(P(\\text{heads})=0.3\\). It is flipped \\(n=5\\) times. Let \\(X\\) = number of heads.\n\n(a) Calculate (P(X=0)=(1-p)^{n}).\n\n(b) Calculate (E(X)=np).\n\n(c) Calculate (text{var}(X)=np(1-p)).",
      a = "(a) \\(P(X=0)=0.7^{5}\\approx0.168\\), (b) \\(E(X)=5\\cdot0.3=1.5\\), (c) \\(\\text{var}(X)=5\\cdot0.3\\cdot0.7=1.05\\).\n\n"
    ),
    list(
      q = "For a binomial distribution with \\(n=10\\) and \\(p=0.5\\):\n\n(a) Calculate (E(X)).\n\n(b) Calculate (text{var}(X)).\n\n(c) Calculate (P(X=0)).",
      a = "(a) \\(E(X)=np=5\\), (b) \\(\\text{var}(X)=np(1-p)=2.5\\), (c) \\(P(X=0)=\\binom{10}{0}0.5^0\\cdot0.5^{10}=(0.5)^{10}\\approx0.001\\).\n\n"
    ),
    list(
      q = "Events occur at a rate of \\(\\lambda=3\\) per hour. \\(X\\) follows a Poisson distribution.\n\n(a) What is (E(X))?\n\n(b) What is (text{var}(X))?\n\n(c) Calculate (P(X=0)=e^{-lambda}).",
      a = "(a) \\(E(X)=3\\), (b) \\(\\text{var}(X)=3\\), (c) \\(P(X=0)=e^{-3}\\approx0.050\\).\n\n"
    ),
    list(
      q = "For a Poisson distribution with \\(\\lambda=2\\):\n\n(a) Calculate (P(X=0)=e^{-2}).\n\n(b) Calculate (P(X=1)=e^{-2}cdotfrac{2^{1}}{1!}).\n\n(c) What is (E(X))?",
      a = "(a) \\(P(X=0)=e^{-2}\\approx0.135\\), (b) \\(P(X=1)=2e^{-2}\\approx0.271\\), (c) \\(E(X)=\\lambda=2\\).\n\n"
    ),
    list(
      q = "Two random variables \\(X\\) and \\(Y\\) are independent with \\(E(X)=3\\), \\(E(Y)=4\\), \\(\\text{var}(X)=2\\), \\(\\text{var}(Y)=5\\).\n\n(a) Calculate (E(X+Y)).\n\n(b) Calculate (text{var}(X+Y)).\n\n(c) What would (text{var}(X+Y)) be if (X) and (Y) were not independent?",
      a = "(a) \\(E(X+Y)=3+4=7\\), (b) \\(\\text{var}(X+Y)=2+5=7\\) (since independent), (c) \\(\\text{var}(X+Y)=\\text{var}(X)+\\text{var}(Y)+2\\,\\text{cov}(X,Y)\\); if not independent the covariance term must be added.\n\n"
    )
  )
