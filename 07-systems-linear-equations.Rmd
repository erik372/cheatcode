...
```{r tva-ekvationer-i, echo=FALSE, out.width="71%", fig.cap="Two equations in one graph"}
library("tidyverse")
g_2lines <- ggplot(tibble(x=-10:10), aes(x)) +
  geom_vline(xintercept=0, alpha=.3) +
  geom_hline(yintercept=0, alpha=.3) +
  theme(axis.title.y=element_text(angle=0), text=element_text(size=11)) +
  stat_function(fun=function(x) 3+2*x, color="#F8766D") +
  stat_function(fun=function(x) x+1, linetype="dashed", color="#00BFC4") +
  geom_point(aes(-2,-1), color="black", size=4) +
  scale_y_continuous(breaks=-5:5, limits=c(-5,5)) +
  scale_x_continuous(breaks=-5:5, limits=c(-5,5)) +
  annotate('text', x=-1.2, y=2, angle=56, size=4, label="y = 3 + 2x") +
  annotate('text', x=1.8, y=2, angle=33, size=4, label="y = x + 1") +
  annotate('text', x=-3.4, y=-.8, size=4, label="(-2,-1)") +
  labs(x="x", y="y")
g_2lines
```
...
```{r mojligt-antal-losningar-linjara-sy, echo=FALSE, out.width="71%", fig.cap="Possible number of solutions for linear systems of equations"}
library("patchwork")
g_onesolution <- ggplot() +
  stat_function(fun=function(x) x, color="#F8766D") +
  stat_function(fun=function(x) 1-x, linetype='dashed', color="#00BFC4") +
  scale_y_continuous(breaks=NULL) +
  scale_x_continuous(breaks=NULL) +
  labs(y=NULL, title="One solution", subtitle="The lines intersect at a point") +
  theme(text=element_text(size=11))

g_nosolution <- ggplot() +
  stat_function(fun=function(x) 1-x, linetype='dashed', color="#F8766D") +
  stat_function(fun=function(x) .7-x, color="#00BFC4") +
  scale_y_continuous(breaks=NULL) +
  scale_x_continuous(breaks=NULL) +
...
g_infsolutions <- ggplot() +
  stat_function(fun=function(x) x, color="#F8766D") +
  stat_function(fun=function(x) .01+x, linetype='dashed', color="#00BFC4") +
  scale_y_continuous(breaks=NULL) +
  scale_x_continuous(breaks=NULL) +
...
```{r 07_systems_linear_equations_ex_2_pdf, echo=FALSE, results='asis'}
if (knitr::is_latex_output()) {
  ex_items <- list(
    list(
      q = "Solve using substitution:\n\n(a) (displaystyle begin{cases} y=3x-1\\ y=x+5 end{cases})\n\n(b) (displaystyle begin{cases} x=2+y\\ x=8-2y end{cases})\n\n(c) (displaystyle begin{cases} y=4x\\ 2x+y=18 end{cases})",
      a = "(a) \\(3x-1=x+5\\Rightarrow x^{*}=3,\\,y^{*}=8\\) , (b) \\(2+y=8-2y\\Rightarrow y^{*}=2,\\,x^{*}=4\\) , (c) \\(2x+4x=18\\Rightarrow x^{*}=3,\\,y^{*}=12\\)\n\n"
    ),
    list(
      q = "For each system, determine whether it has a unique solution, infinitely many solutions, or no solution:\n\n(a) (displaystyle begin{cases} y=2x+1\\ y=2x+4 end{cases})\n\n(b) (displaystyle begin{cases} y=3x-2\\ 2y=6x-4 end{cases})\n\n(c) (displaystyle begin{cases} y=x+1\\ y=-x+5 end{cases})",
      a = "(a) Same slope, different intercept: no solution, (b) Second equation is \\(y=3x-2\\) , same as first: infinitely many solutions, (c) Unique solution: \\(x^{*}=2,\\,y^{*}=3\\)\n\n"
    ),
    list(
      q = "Solve using row reduction:\n\n(a) (displaystyle begin{cases} x+y=7\\ 2x-y=2 end{cases})\n\n(b) (displaystyle begin{cases} 2x+y=9\\ x-y=0 end{cases})\n\n(c) (displaystyle begin{cases} x+2y=8\\ 3x-y=3 end{cases})",
      a = "(a) Add rows: \\(3x=9\\Rightarrow x^{*}=3,\\,y^{*}=4\\) , (b) Add rows: \\(3x=9\\Rightarrow x^{*}=3,\\,y^{*}=3\\) , (c) Row 2 \\(\\times2\\) then add to row 1: \\(7x=14\\Rightarrow x^{*}=2,\\,y^{*}=3\\)\n\n"
    ),
    list(
      q = "In a market for a good, demand is given by \\(p=100-2q\\) and supply by \\(p=10+3q\\) , where \\(p\\) is price and \\(q\\) is quantity.\n\n(a) Set up the system of equations.\n\n(b) Find the equilibrium price and quantity.\n\n(c) At (q=15) , calculate demand price and supply price. Is there excess supply or excess demand?",
      a = "(a)\n\n\\(\\displaystyle \\begin{cases} p=100-2q\\\\ p=10+3q \\end{cases}\\)\n\n, (b) \\(100-2q=10+3q\\Rightarrow q^{*}=18,\\,p^{*}=64\\) , (c) \\(p_{D}(15)=70,\\,p_{S}(15)=55\\) : excess demand (demand price exceeds supply price)\n\n"
    ),
    list(
      q = "Solve using logarithms where needed:\n\n(a) (displaystyle begin{cases} y+7=4\\ 2^{x}-3y=13 end{cases})\n\n(b) (displaystyle begin{cases} log x=2\\ log x+log y=5 end{cases})\n\n(c) (displaystyle begin{cases} 5^{x}=25\\ y=2x-1 end{cases})",
      a = "(a) \\(y^{*}=-3\\) ; \\(2^{x}-3(-3)=13\\Rightarrow2^{x}=4\\Rightarrow x^{*}=2\\) , (b) \\(x^{*}=100\\) ; \\(\\log100+\\log y=5\\Rightarrow\\log y=3\\Rightarrow y^{*}=1000\\) , (c) \\(5^{x}=5^{2}\\Rightarrow x^{*}=2,\\,y^{*}=3\\)\n\n"
    ),
    list(
      q = "Solve the following systems with three variables:\n\n(a) (displaystyle begin{cases} z=2\\ y=z+3\\ x=y+1 end{cases})\n\n(b) (displaystyle begin{cases} x+y+z=12\\ x=3y\\ z=2y end{cases})\n\n(c) (displaystyle begin{cases} x+y=7\\ y+z=8\\ x+z=9 end{cases})",
      a = "(a) \\(z^{*}=2,\\,y^{*}=5,\\,x^{*}=6\\) , (b) \\(3y+y+2y=12\\Rightarrow y^{*}=2,\\,x^{*}=6,\\,z^{*}=4\\) , (c) \\(x+y+z=12\\Rightarrow z^{*}=5,\\,x^{*}=4,\\,y^{*}=3\\)\n\n"
    ),
    list(
      q = "Check whether the given values are solutions to each system:\n\n(a) (x^{*}=2,,y^{*}=3) for(displaystyle begin{cases} y=x+1\\ 2x+y=7 end{cases})\n\n(b) (x^{*}=1,,y^{*}=2) for(displaystyle begin{cases} 3x+y=4\\ x-y=0 end{cases})\n\n(c) (x^{*}=4,,y^{*}=-1) for(displaystyle begin{cases} x+2y=2\\ 3x-y=13 end{cases})",
      a = "(a) \\(3=2+1\\) and \\(4+3=7\\) : yes, (b) \\(3(1)+2=5\\neq4\\) : no, (c) \\(4+2(-1)=2\\) and \\(12-(-1)=13\\) : yes\n\n"
    ),
    list(
      q = "Solve using row reduction or substitution:\n\n(a) (displaystyle begin{cases} x+y+z=6\\ x-y+z=2\\ x+y-z=2 end{cases})\n\n(b) (displaystyle begin{cases} x=3\\ y=x+2\\ z=x+y end{cases})\n\n(c) (displaystyle begin{cases} 2x+y=10\\ y+z=7\\ x+z=6 end{cases})",
      a = "(a) Row 2 \\(-\\) Row 1: \\(y^{*}=2\\) ; Row 3 \\(-\\) Row 1: \\(z^{*}=2\\) ; \\(x^{*}=2\\) , (b) \\(x^{*}=3,\\,y^{*}=5,\\,z^{*}=8\\) , (c) \\(x^{*}=3,\\,y^{*}=4,\\,z^{*}=3\\)\n\n"
    ),
    list(
      q = "Three friends A, B and C together spent 240 on a dinner. A spent twice as much as B. C spent 40 more than B.\n\n(a) Set up a system of equations.\n\n(b) Solve for what each person paid.\n\n(c) What percentage share of the total did each person pay?",
      a = "(a)\n\n\\(\\displaystyle \\begin{cases} A+B+C=240\\\\ A=2B\\\\ C=B+40 \\end{cases}\\)\n\n, (b) \\(2B+B+B+40=240\\Rightarrow B^{*}=50,\\,A^{*}=100,\\,C^{*}=90\\) , (c) A: \\(100/240\\approx41.7\\%\\) , B: \\(50/240\\approx20.8\\%\\) , C: \\(90/240=37.5\\%\\)\n\n"
    ),
    list(
      q = "In a simple labor market, labor demand is \\(L_{D}=200-5w\\) and labor supply is \\(L_{S}=20+3w\\) , where \\(w\\) is the wage.\n\n(a) Find the equilibrium wage and employment level.\n\n(b) A minimum wage of 25 is introduced. Calculate (L_{D}) and (L_{S}) at this wage.\n\n(c) Is there excess supply or excess demand at the minimum wage, and by how many workers?",
      a = "(a) \\(200-5w=20+3w\\Rightarrow w^{*}=22.5,\\,L^{*}=87.5\\) , (b) \\(L_{D}(25)=75,\\,L_{S}(25)=95\\) , (c) Excess supply (unemployment) of \\(95-75=20\\) workers\n\n"
    )
  )
